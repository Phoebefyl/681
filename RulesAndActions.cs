...
namespace DepAnalysis
{
  ...
  public class Repository
  {
    ...
    public Dictionary<string, List<List<string>>> usingTable { get; set; } = new Dictionary<string, List<List<string>>>();
    public CsGraph<string, string> depGraph { get; set; } = new CsGraph<string, string>("depGraph");
    ...
    public class PushTypeTable : AAction
  {
    public PushTypeTable(Repository repo)
    {
      repo_ = repo;
    }
    ...
    public override void doAction(ITokenCollection semi)
    {
      ...
      if (elem.type == "control" || elem.type == "namespace" || elem.type =="function")
        return;

            TypeItem typeItem = new TypeItem();
            typeItem.file = semi.filePath;

            List<string> scope = new List<string>();
            for(int i = 0; i <repo_.stack.count-1; i++)
            {
                scope.Add(repo_.stack[i].name);
            }

            typeItem.namesp = scope;
            typeItem.category = semi[0];

            repo_.typeTable.add(semi[1],typeItem);
  ...
  public class DetectClass : ARule
  {
    ...
    public class DetectDelegate : ARule
    {
        public override bool test(ITokenCollection semi)
        {
            ...
            semi.find(";", out indexSC);

            if (indexDG != -1 && indexSC!=-1 && semi.size() > indexDG + 1)
            {
                ITokenCollection local = Factory.create();
                local.filePath = semi.filePath;
                // local semiExp with tokens for type and name
  ...
  public class BuildCodeAnalyzer
  {
    ...
    public BuildCodeAnalyzer(Repository rep)
    {
            repo = rep;
            //repo.semi = semi;
    }
    public virtual TypeAnalysis build()
    {
      TypeAnalysis parser = new TypeAnalysis();
      ...
      PushTypeTable pushTypeTable = new PushTypeTable(repo);

      // capture namespace info
      DetectNamespace detectNS = new DetectNamespace();
      detectNS.add(pushTypeTable);
      parser.add(detectNS);

      // capture class info
      DetectClass detectCl = new DetectClass();
      detectCl.add(pushTypeTable);
      parser.add(detectCl);

      // capture delegate indo
     DetectDelegate detectDelegate = new DetectDelegate();
     detectDelegate.add(pushTypeTable);
     parser.add(detectDelegate);

      // capture function info
      DetectFunction detectFN = new DetectFunction();
      detectFN.add(pushTypeTable);
      parser.add(detectFN);

      // handle entering anonymous scopes, e.g., if, while, etc.
      DetectAnonymousScope anon = new DetectAnonymousScope();
      anon.add(pushTypeTable);
      parser.add(anon);
